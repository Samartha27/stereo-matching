...
def main():
    ...
    parser.add_argument("-rk", help="Input only the session and camera directories of left keypoints",  required=True)
    ...
    points2 = np.load(args.rk)

    F = np.load(args.F)

    utils.draw_epipolar_lines(im1, im2, points1, points2, F)
